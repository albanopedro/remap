...
def cmd_apply_profile(args):
    ...
    # Selecionar perfilni
    if args.profile.lower() in PRESET_PROFILES:
        profile = PRESET_PROFILES[args.profile.lower()]
        print(f"[✓] Usando perfil predefido: {profile.name}")
    else:
    ...
    print(f"    Potência estimada: +{gains['power_gain_percent']:.2f}%")
    print(f"    Torque estimado: +{gains['torque_gain_percent']:.2f}%")
    ...
    try:
            ...
            return 1
    if args.report:
        report_path = args.report or output_path.replace('.bin', '_report.txt')
        ecu.export_report(report_path)
        print(f"[✓] Relatório exportado: {report_path}")
...
def cmd_list_profiles(args):
    ...
    for profile_name, profile in PRESET_PROFILES.items():
        ...
        gains = ecu.calculate_performance_gain(profile)
        print(f"  Ganhos:       +{gains['power_gain_percent']:.2f}% potência, +{gains['torque_gain_percent']:.2f}% torque")
    
    print("\n" + "="*70 + "\n")
    return 0, validate=not args.skip_validation
...
def cmd_estimate(args):
    ...
    print(f"\n[✓] Ganhos Estimados:")
    print(f"    Potência: +{gains['power_gain_percent']:.2f}% ({gains['estimated_power']:.0f}hp)")
    print(f"    Torque: +{gains['torque_gain_percent']:.2f}% ({gains['estimated_torque']:.0f}Nm)")
    print()
...
Exemplos de uso:
  ...
  python cli.py backup dump_ecu.bin
    ...
    subparsers = add_argument('--skip-validation', action='store_true', default=False,
                             help='Pular validação de header ECU')
    apply_parser.parser.add_subparsers(dest='command', help='Comandos disponíveis')
    ...
    apply_parseradd_argument('--skip-validation', action='store_true', default=False,
                             help='Pular validação de header ECU')
    info_parser..set_defaults(func=cmd_apply_profile)
